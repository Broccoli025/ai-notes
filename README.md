...
- **Markdown 渲染 + 代码高亮**：预览模式渲染 Markdown，代码块带语法高亮；双击预览进入编辑，`⌘E` 切换。
...
- **导出 Word**：当前笔记 `⌘⇧E` 一键导出 .docx；「文件 → 导出当前列表为 Word」把筛选出的多条笔记合成一个文档。标题、列表、表格、代码块、引用、链接都会映射成 Word 样式。
...
src/preload.js           渲染进程可用的安全 API、Markdown 渲染
src/renderer/            主窗口与快速收藏窗口的界面
```
